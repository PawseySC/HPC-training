...
void updatePos(Body* __restrict__ body, const FLOAT dt, const int n, const int my_n) 
{
#pragma omp parallel for schedule(guided)
	...
	MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, body->x,  my_n, MPIFLOAT, MPI_COMM_WORLD);
	MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, body->y,  my_n, MPIFLOAT, MPI_COMM_WORLD);
	MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, body->z,  my_n, MPIFLOAT, MPI_COMM_WORLD);
...
FLOAT energy(Body* __restrict__ body, const int n)
{
	...
	FLOAT energy;
	MPI_Reduce(&enr, &energy, 1, MPIFLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
	return energy/TWO;
}
...
// the main routine
void bodyForce(Body *p, FLOAT const dt, const int n, const int my_n, const int size, const int rank) 
{
	//long int count = 0;
	int sp = (rank + 0)*my_n;
	int ep = (rank + 1)*my_n;
...
#pragma omp parallel for schedule(guided) 
	for (int ii = sp; ii < ep; ii += TILE_SIZE) 
	{ 
		...
		for (int jj = 0; jj < n; jj += 1) 
			for (int jb = ii; jb < ii + TILE_SIZE; jb += 1) 
			{
				...
				const FLOAT invSqrtDist = INVSQRT(dx*dx + dy*dy + dz*dz + (FLOAT) SOFTENING);                     // 2 fLops (?)
				//
...
int main(const int argc, const char** argv) 
{
	...
	//
	MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, p.x,  my_nBodies, MPIFLOAT, MPI_COMM_WORLD);
	MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, p.y,  my_nBodies, MPIFLOAT, MPI_COMM_WORLD);
	MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, p.z,  my_nBodies, MPIFLOAT, MPI_COMM_WORLD);
	MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, p.vx, my_nBodies, MPIFLOAT, MPI_COMM_WORLD);
	MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, p.vy, my_nBodies, MPIFLOAT, MPI_COMM_WORLD);
	MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, p.vz, my_nBodies, MPIFLOAT, MPI_COMM_WORLD);
	//
	if (rank == 0)
	{
	...
	for (int iter = 1; iter <= nIters; iter++) 
	{
		...
		{
			bodyForce(&p, dt, nBodies, my_nBodies, size, rank); // compute interbody forces
			updatePos(&p, dt, nBodies, my_nBodies);
		}
		unsigned long c1 = rdpmc_actual_cycles();
		time += myseconds();
		totalTime += time;
		if (rank == 0) printf("%4d %10.3e %10.3e    \033[42m%.1f\033[0m    %3.1f\n", iter, time,  (double) nBodies*nBodies/time, 21.*nBodies*nBodies*1e-9/time, 30.*nBodies*nBodies/((double) c1 - c0));
	}
